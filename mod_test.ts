...
Deno.test({
  ...
  async fn() {
    ...
    await assertRejects( () => checkCopyrightHeaders(options), Error, "Invalid first year");
    await assertRejects( () => updateCopyrightHeaders(options), Error, "Invalid first year");
    ...
    options.extensions = [];
    await assertRejects( () => checkCopyrightHeaders(options), Error, "No extensions provided");
    await assertRejects( () => updateCopyrightHeaders(options), Error, "No extensions provided");

    options.firstYear = new Date().getFullYear();
    options.extensions = [".java", ".jsx"];
    options.rootDir = "";
    await assertRejects( () => checkCopyrightHeaders(options), Error, "No root directory provided");
...
Deno.test({
  ...
  async fn(t) {
    const license = "// Copyright 2018-{TIMEFRAME}. MIT license.".replace("{TIMEFRAME}", new Date().getFullYear().toString());
    const licenseOld = "// Copyright 2018-{TIMEFRAME}. MIT license.".replace("{TIMEFRAME}", (new Date().getFullYear() - 1).toString());
    await Deno.mkdir("testdir");
    Deno.writeFileSync("my_file.test", new TextEncoder().encode("hello world\n"));
    ...
    await t.step("Check files with first year specified", async () => {
      ...
      options.exclusions = ["**/my_file2.test", "**/my_file4.test", "my_file.test"];
      assert(await checkCopyrightHeaders(options));
    });
    ...
    await t.step("Update files with first year specified", async () => {
      ...
      assert(await checkCopyrightHeaders(options));
      assertEquals(Deno.readTextFileSync("my_file.test"), license + "\nhello world\n");
    });
...
Deno.test({
  ...
  async fn(t) {
    const license = "// Copyright {TIMEFRAME}. MIT license.".replace("{TIMEFRAME}", new Date().getFullYear().toString());
    const licenseOld = "// Copyright {TIMEFRAME}. MIT license.".replace("{TIMEFRAME}", (new Date().getFullYear() - 1).toString());
    await Deno.mkdir("testdir");
    Deno.writeFileSync("my_file.test", new TextEncoder().encode("hello world\n"));
    ...
    await t.step("Check and update files with first year specified", async () => {
      const options:Options = {
        extensions: [".test"],
        exclusions: [],
        headerText: `// Copyright {TIMEFRAME}. MIT license.`,
