 ...
 */
export async function checkCopyrightHeaders(
  options: Options,
): Promise<boolean> {
  const [filesMissingLicense, filesWithOutOfDateLicense] = await walkFiles(
    options,
    false,
  );
  outputCheckResult(
    filesMissingLicense,
    filesWithOutOfDateLicense,
    options.quiet,
  );
  return filesMissingLicense.length == 0 &&
    filesWithOutOfDateLicense.length == 0;
}
...
async function walkFiles(
  ...
  const { extensions, exclusions = [], firstYear, headerText, rootDir } =
    options;

  const timeframe = (firstYear && firstYear != CURRENT_YEAR)
  ...
  for await (
    ...
    if (content.indexOf(license) == -1) {
      if (
        content.indexOf(licenseStart) >= 0 && content.indexOf(licenseEnd) >= 0
      ) {
        filesWithOutOfDateLicense.push(path);
        if (shouldUpdate) {
          const licenseStartIndex = content.indexOf(licenseStart);
          const licenseEndIndex = content.indexOf(licenseEnd) +
            licenseEnd.length;
          const updatedContent = content.substring(0, licenseStartIndex) +
            license + content.substring(licenseEndIndex);
...
function outputCheckResult(
  filesMissingLicense: string[],
  filesWithOutOfDateLicense: string[],
  ...
  if (filesMissingLicense.length > 0 || filesWithOutOfDateLicense.length > 0) {
    if (!quiet) {
      ...
      if (filesWithOutOfDateLicense.length > 0) {
        ...
        console.log("------------------------------");
        console.log(
          filesWithOutOfDateLicense.map((path) => `  ${path}`).join("\n"),
        );
      }
      console.log("");
    }
    ...
    if (!quiet) {
      console.log(
        "%cAll files have valid up to date copyright header",
        "color: green",
      );
    }
  }
...
function outputUpdateResult(
  ...
  if (filesMissingLicense.length > 0 || filesWithOutOfDateLicense.length > 0) {
    ...
    if (!quiet) {
      console.log(
        "%cNo updates necessary, all files have valid up to date copyright/license header",
        "color: green",
      );
    }
  }
...
function validateOptions(options: Options, currentYear: number): void {
  if (options.firstYear) {
    assert(
      options.firstYear > 1900 && options.firstYear <= currentYear,
      "Invalid first year",
    );
  }
